...
| D10 | Child env always stripped of `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN` and the Bedrock/Vertex/Foundry switches; if the SDK system message reports `apiKeySource` other than `none`, refuse the session with a reason. API-key billing keeps its existing path: dsh's stock Anthropic provider. Rate-limit events are published as provider-neutral `provider/status` session events, folded by one host projection `providerStatus`, and shown live as a small pill in the composer row beside the model name through the standard `useProjection` slot prop — the same channel every other provider can feed (plan Tasks 13–14); cost figure hidden; Fable cap trips → `fallbackModel` Opus with a visible note. No separate usage meter page. |

## Implementation status

- Plan: `docs/plans/claude-subscription-engine-implementation.md` (Tasks 1–14).
- Shipped (Tasks 1–12): the claude-code engine — `plugins/claude-code/lib/{models,env,probe,spawn,pending,mirror-tools,bridge,handoff,mcp-bridge,approval,adapter,auth-flow}.js`, `index.mjs` (adapter registration, sign-in flow, pi-ai OAuth hide) and `agent.mjs` (mirror tools). 54 offline test suites green. `scripts/claude-code-smoke.mjs` (Task 12) is the hand-run live smoke that exercises all of the above against a real signed-in binary; it never runs in CI.
- Not yet shipped: the D10 `provider/status` channel and composer pill (Tasks 13–14) — designed in this doc's decision table but not implemented in code yet.
