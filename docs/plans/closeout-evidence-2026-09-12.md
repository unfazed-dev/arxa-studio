...
| G10 | Omarchy VM + container lanes (T15) | operator-machine | runbook + assertions in arxa `docs/linux-support.md`; container lane `DISTRO=ubuntu scripts/linux/run-container.sh` | studio 8e3bc01 | EXTERNAL (owner: operator) | — | operator VM + daemon; first `desktop/**` PR runner pass (AXS-053) |
