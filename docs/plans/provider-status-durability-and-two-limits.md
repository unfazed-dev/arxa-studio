...
## Thirteenth (2026-09-06): Models page "settings are unavailable in this browser"

- **Symptom.** Settings → Models: "Loading the provider directory failed: settings are unavailable in this browser".
- **Trace.** dsh-client-ui-settings-models `load()` shows that fallback when the settings mirror holds no view and no error. The mirror is built with persistence `memory` when `remote.$host.isLoopback` is false, and in memory mode `ensure()` resolves without ever reading. isLoopback comes from dsh-client-connection's `isLoopbackHostname`, which arxa widens to `*.localhost` via `bin/loopback-localhost-patch.mjs` so the desktop page at arxa.studio.localhost counts as loopback.
- **Root cause.** The patch hooks `fs.readFileSync` / `fs.promises.readFile`, but dsh-client-modules reads bundles through `import { readFileSync } from "node:fs"`, a named ESM binding that an assignment on the fs object does not move. Verified: a `--import` probe showed the named import unpatched; the served combo bundle at :7891 still carried the stock predicate.
- **Fix.** `syncBuiltinESMExports()` after the assignments (bin/loopback-localhost-patch.mjs). New `bin/selftest.loopback-patch.mjs` (wired in scripts/ci.mjs) reads a fake bundle through a named import under the patch and fails without the sync.
- **Verified.** Served bundle now carries the widened predicate at the same byte length; Models page lists DeepSeek, kimi-coding and zai.
