...
### §10.5 — scope, migration, and a known ceiling

**The org tier was never affected.** `orgCheckSh()` is a separate function and
was deliberately left untouched: it performs structural checks only (org.json
parses, the five fixed docks, `NN-kebab` stage names, no stray locks/temp files)
and has **no stack probe at all**. B11 was never live there. Confirmed by
measurement: RESTO, TESTO and TOPO hold **0 stack markers outside `projects/`**,
and the org `.gitignore` `/*` allowlist excludes `projects/` anyway (D37). All
three org `check.sh` files are byte-identical to the current `orgCheckSh()`.
B11/B15/B16 are project-tier only.

**Migration — the fix does not reach existing repos on its own.** By design
`writeFrameFiles` never clobbers a `check.sh` that already exists (asserted in
`selftest.mjs`). project-001 is current only because it was regenerated by hand
during this work. Any project created before this fix keeps the old root-only
gate **forever**, silently. Before this is called done, the rewire needs an
explicit regeneration path — a version stamp inside the generated `check.sh` and
a "frame out of date" action — or the fix is built-but-unwired, the same pattern
named five times elsewhere in this plan.

**Paths with spaces are safe.** A target at `development/my track/ios/todo_app`
reds correctly and reports its path intact. Track and target names are
user-supplied, so this is a real case, and it is covered.

**Known ceiling, not a fix.** One Flutter target costs ~84 s (analyze + 169
tests) against `timeout-minutes: 15`. At roughly six such targets per project the
job hits the wall. Options when that day comes: raise the timeout, shard by
target across jobs, or run tests only for targets touched by the diff. Not
actioned now — recorded so it is a decision later rather than a surprise.
