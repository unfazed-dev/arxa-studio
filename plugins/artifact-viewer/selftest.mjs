...
// D89/D90 lanes exist server-side
{
  ...
  assert.match(t5client, /aXa_av_colHost\{[^}]*border-left:\.5px solid var\(--dsw-alias-border-l4\)/, 'the owned column keeps the stock divider (P3OORG_panel border-left parity)')
  assert.match(t5client, /\[data-rightbar-fullscreen\] \.aXa_av_colHost\{border-left:none\}/, 'fullscreen drops the divider — stock parity')
  ...
  assert.doesNotMatch(t5client, /data-conversation-scroll/, 'an empty strip ALWAYS collapses the track — the conversation special-case died with the empty-column look (2026-09-16)')
  ...
  assert.doesNotMatch(t5client, /colNode,\s*edgeChip\)/, "the chip is NOT createPortal's third arg — that slot is the React key, and the chip silently never mounted (2026-09-15)")
