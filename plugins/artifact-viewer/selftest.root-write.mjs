...
try {
  ...
  assert.equal(fs.readFileSync(path.join(root.path, body.relPath), 'utf8'), body.content)
  const target = path.join(root.path, body.relPath)
  fs.writeFileSync(target, 'external half-millisecond change\n')
  fs.utimesSync(target, (mtime + 0.5) / 1000, (mtime + 0.5) / 1000)
  const externalMtime = fs.statSync(target).mtimeMs
  assert.notEqual(externalMtime, mtime, 'fixture records a distinct fractional mtime')
  assert.ok(Math.abs(externalMtime - mtime) <= 1, 'fixture exercises the sub-millisecond conflict window')
  result = await write({ ...body, content: 'must preserve external', expectedMtimeMs: mtime })
  assert.equal(result.status, 409)
  assert.equal(fs.readFileSync(target, 'utf8'), 'external half-millisecond change\n')
  result = await write({ ...body, content: 'second save', expectedMtimeMs: externalMtime })
  assert.equal(result.status, 200)
  fs.unlinkSync(target)
  assert.equal((await write({ ...body, content: 'must not recreate', expectedMtimeMs: result.mtimeMs })).status, 409)
  assert.equal(fs.existsSync(path.join(root.path, body.relPath)), false, 'stale save does not recreate an externally deleted file')
  assert.equal((await write(body)).status, 200, 'explicit overwrite/new save can recreate without an expected mtime')
