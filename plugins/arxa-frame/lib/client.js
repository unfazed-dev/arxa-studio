	...
	factory: (require) => {
		...
		*/
		function computeColumns(viewport, sidebar, details, viewer) {
			...
			const v0 = viewer === 0 ? 0 : clampWidth(viewer, 320, 100000);
			if (s + d0 + v0 + 640 <= viewport) return {
				sidebar: s,
				center: viewport - s - d0 - v0,
				details: d0,
				viewer: v0
			};
			const d1 = d0 === 0 ? 0 : Math.min(d0, Math.max(300, viewport - s - v0 - 640));
			if (s + d1 + v0 + 640 <= viewport) return {
				sidebar: s,
				center: 640,
				details: d1,
				viewer: v0
			};
			const v1 = Math.min(v0, Math.max(320, viewport - s - 640));
			if (s + v1 + 640 <= viewport) return {
				sidebar: s,
				center: 640,
				details: 0,
				viewer: v1
		...
		const css = ".aXa_fr_frame{background:var(--dsw-alias-bg-base);height:100%;transition:grid-template-columns var(--ds-transition-duration-slow) var(--ds-ease-in-out);grid-template-rows:100%;display:grid;position:relative;overflow:hidden}.aXa_fr_frame[data-dragging]{transition:none}@media (prefers-reduced-motion:reduce){.aXa_fr_frame{transition:none}}.aXa_fr_sidebarCol{background:var(--dsw-specific-sidebar-fill);border-right:1px solid var(--dsw-alias-border-l1);min-width:0;overflow:hidden}.aXa_fr_centerCol{flex-direction:column;min-width:0;display:flex;overflow:hidden}.aXa_fr_detailsCol{border-left:1px solid var(--dsw-alias-border-l2);min-width:0;overflow:hidden}.aXa_fr_frame[data-details-collapsed] .aXa_fr_detailsCol{border-left:none}.aXa_fr_handle{cursor:col-resize;z-index:2;touch-action:none;width:8px;transition:left var(--ds-transition-duration-slow) var(--ds-ease-in-out);margin-left:-4px;position:absolute;top:0;bottom:0}.aXa_fr_frame[data-dragging] .aXa_fr_handle{transition:none}@media (prefers-reduced-motion:reduce){.aXa_fr_handle{transition:none}}.aXa_fr_handle[data-side=details]:after{content:\"\";box-sizing:border-box;background:var(--dsw-alias-button-floating-fill);border:1px solid var(--dsw-alias-border-l2-darkmode-thin);opacity:0;width:12px;height:32px;transition:opacity var(--ds-transition-duration-slow) var(--ds-ease-in-out), background var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:10px;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)}.aXa_fr_detailsCol:hover~.aXa_fr_handle[data-side=details]:after,.aXa_fr_handle[data-side=details]:hover:after,.aXa_fr_handle[data-side=details][data-dragging=true]:after{opacity:1}.aXa_fr_handle[data-side=details]:hover:after,.aXa_fr_handle[data-side=details][data-dragging=true]:after{background:var(--dsw-alias-button-floating-hover);border-color:var(--dsw-alias-border-l3)}.aXa_fr_overlayLayer{z-index:20;pointer-events:none;position:absolute;inset:0}.aXa_fr_overlayLayer>*{pointer-events:auto}.aXa_fr_viewerCol{border-left:1px solid var(--dsw-alias-border-l2);min-width:0;overflow:hidden}.aXa_fr_frame[data-viewer-collapsed] .aXa_fr_viewerCol{border-left:none}.aXa_fr_handle[data-side=viewer]:after{content:\"\";box-sizing:border-box;background:var(--dsw-alias-button-floating-fill);border:1px solid var(--dsw-alias-border-l2-darkmode-thin);opacity:0;width:12px;height:32px;transition:opacity var(--ds-transition-duration-slow) var(--ds-ease-in-out),background var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:10px;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)}.aXa_fr_viewerCol:hover~.aXa_fr_handle[data-side=viewer]:after,.aXa_fr_handle[data-side=viewer]:hover:after,.aXa_fr_handle[data-side=viewer][data-dragging=true]:after{opacity:1}.aXa_fr_handle[data-side=viewer]:hover:after,.aXa_fr_handle[data-side=viewer][data-dragging=true]:after{background:var(--dsw-alias-button-floating-hover);border-color:var(--dsw-alias-border-l3)}.aXa_fr_sheetLayer{position:absolute;inset:0;z-index:15;display:flex;flex-direction:column;background:var(--dsw-alias-bg-base)}.aXa_fr_sheetHead{display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid var(--dsw-alias-border-l2)}.aXa_fr_sheetBack{cursor:pointer;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-primary);border-radius:6px;padding:4px 10px;font-size:12px}.aXa_fr_sheetBody{flex:1;min-height:0;display:flex}";
		...
		var AppFrame_module_css_default = {
			"centerCol": "aXa_fr_centerCol",
			"detailsCol": "aXa_fr_detailsCol",
			"frame": "aXa_fr_frame",
		...
		*/
		...
		function AppFrame({ useStore, useSessions, actions, renderSlot }) {
			...
			const cols = computeColumns(viewport, sidebarCollapsed ? 0 : panels.sidebar === 0 ? 280 : panels.sidebar, detailsSession === void 0 ? 0 : panels.details, panels.viewer);
			...
			const viewerBase = (0, react.useRef)(0);
			const onViewerStart = (0, react.useCallback)(() => {
				viewerBase.current = colsRef.current.viewer;
				setDragging(true);
			}, []);
			const onViewerDrag = (0, react.useCallback)((dx) => {
				actions.setViewer(viewerBase.current + dx);
			}, [actions]);
			const requestViewerMax = (0, react.useCallback)(() => {
				const c2 = colsRef.current;
				let room = viewport - c2.sidebar - (c2.details > 0 ? c2.details : 0) - 640;
				if (room < 320 && c2.details > 0) {
					actions.closeDetails();
					room = viewport - c2.sidebar - 640;
				}
				actions.setViewer(Math.max(320, room));
			}, [actions, viewport]);
				...
				children: [
					...
					narrow && (detailsSession !== void 0 || panels.viewer > 0) && panels.viewer > 0 && (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [(0, react_jsx_runtime.jsx)("div", {
							...
							children: (0, react_jsx_runtime.jsx)("button", { className: AppFrame_module_css_default.sheetBack, onClick: () => actions.closeViewer(), children: "← artifacts" })
						}), (0, react_jsx_runtime.jsx)("div", {
							className: AppFrame_module_css_default.sheetBody,
							children: renderSlot("viewer", { close: actions.closeViewer, maximize: null, sheet: true })
						})]
					}),
					(0, react_jsx_runtime.jsx)("div", {
