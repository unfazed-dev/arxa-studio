		...
		* the stock renderer read as ages from 1970. */
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=arxa-sidebar-empty-state]") === null) {
			...
			tag.textContent = "[data-arxa-hero-guide]{font-size:12.5px;opacity:.72;line-height:1.55;max-width:470px}"
				...
				// G3 (grilled 2026-09-06): the row the current session lives on gets
				// an accent dot beside the stock selected background — the folder
				// glyph above it already lights (dsh folderActive), so the pair reads
				// as "this branch, this row". Painted as a background gradient, NOT a
				// pseudo-element: ::before and ::after on .sessionRow are taken by the
				// drag markers. The extra `div` step outranks the stock rule without
				// depending on which style tag lands last.
				+ "div.aXa_wsr_sessionRow.aXa_wsr_selected{background-color:var(--dsw-alias-interactive-bg-hover);background-image:radial-gradient(circle at 3px 50%,var(--dsw-alias-state-business-primary) 0 2.5px,transparent 2.6px);background-repeat:no-repeat}"
