...
// The original package stays untouched in node_modules as reference.
		...
		*/
		function ProjectRowItem({ group, onToggle, onCreate, actions, drag, home, t }) {
			...
			// AXS-003 closeout: subscribe to the deco map, like every decorated row.
			ARXA_USE_DECO();
			...
			const ownRow = (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("span", {
						...
						children: (0, react_jsx_runtime.jsx)("span", {
							// AXS-003 closeout: the folded dir mark on a leaf workspace row.
							className: clsx(Rows_module_css_default.title, ARXA_WS_DECO_CLASS(row.workspaceId)),
							children: label
						})
					}),
					ARXA_DECO_BADGE(wsParts(row.workspaceId).ws, "dir"),
					(0, react_jsx_runtime.jsxs)("span", {
		...
		* the stock renderer read as ages from 1970. */
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=arxa-sidebar-empty-state]") === null) {
			...
			tag.textContent = "[data-arxa-hero-guide]{font-size:12.5px;opacity:.72;line-height:1.55;max-width:470px}"
				...
				+ ".aXa_fs_body{display:flex;min-height:0;flex:1;flex-direction:column;padding-right:12px}.aXa_fs_empty{padding:18px 14px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xs-13)}"
				+ ".aXa_fs_roots{min-height:0;overflow:auto;padding:6px 0 6px 4px}.aXa_fs_tree{padding-left:10px}"
		...
		/** AXS-003 closeout: the folded dir mark for a stock LEAF workspace row
		 * — a stage container ('projects/<slug>/01-discovery') or any other
		 * leaf folder. Same map, same letters as the leaf/lister rows; null
		 * when nothing under it changed. Returns the class pair so the
		 * generated row can slot it straight into clsx(). */
		const ARXA_WS_DECO_CLASS = (workspaceId) => {
			const letter = ARXA_DECO_FOR(wsParts(workspaceId).ws, "dir");
			return letter ? "aXa_decoText aXa_deco_" + letter : null;
		};
		...
		* directory picker flow stays permanently unavailable. */
		...
		function OrgContainerRow({ d, offset }) {
			...
			// AXS-003 closeout: subscribe to the deco map — without this the
			// top-level mark waits for the next poll to paint.
			ARXA_USE_DECO();
			/* AXS-003 closeout: the Projects dock row and each project row are
			 * the top of the tree — they fold the SAME map the leaf rows read
			 * (foldDirs already rolls every ancestor up), so wiring them adds
			 * zero git work. Org rows never decorate: the org is not a folder
			 * in any session's diff. */
			const decoTop = !isOrg && d.kind === "dock" && d.slug === "projects" ? "projects" : d.kind === "project" ? "projects/" + d.slug : null;
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("span", {
						className: Rows_module_css_default.projectText,
						children: (0, react_jsx_runtime.jsx)("span", { className: clsx(Rows_module_css_default.title, decoTop && ARXA_DECO_FOR(decoTop, "dir") && "aXa_decoText", decoTop && ARXA_DECO_FOR(decoTop, "dir") && ("aXa_deco_" + ARXA_DECO_FOR(decoTop, "dir"))), children: d.label })
					}),
					// AXS-003 closeout: the folded mark on the Projects /
					// project rows — the same badge the leaf rows carry.
					decoTop ? ARXA_DECO_BADGE(decoTop, "dir") : null,
...
"menu.org.trash": "Move to Trash",
			...
			"freestyle.section": "Repositories",
			...
			"freestyle.trash.folderSection": "Repositories",
		...
		"menu.org.trash": "Przenieś do Kosza",
			...
			"freestyle.trash.folderSection": "Repozytoria",
		...
		"menu.org.trash": "Déplacer vers la corbeille",
			...
			"rename.done": "Renommé",
			"rename.close": "Fermer",
			"rename.orgNote": "Le dossier de l’organisation et son dépôt GitHub sont renommés ensemble.",
			...
			"archives.toTrash": "Déplacer vers la corbeille",
			"trash.sessionSection": "Sessions",
			...
			"purge.orgWarn": "Ceci supprime définitivement le dossier de l’organisation ET tous les dépôts GitHub qu’elle possède — le dépôt de l’organisation et chaque dépôt de projet publié. Cette action est irréversible.",
			"purge.projectWarn": "Ceci supprime définitivement le dossier de la corbeille ET le dépôt GitHub du projet. Cette action est irréversible.",
			"purge.confirm": "Supprimer définitivement",
			"purge.busy": "Suppression des dépôts GitHub et du dossier de la corbeille…",
			...
			"github.signin.desc": "arxa studio gère tout via git. Liez votre compte GitHub pour créer une organisation — vos projets sont publiés comme des dépôts privés sous elle.",
			...
			"freestyle.trash.folderSection": "Dépôts",
