...
// The original package stays untouched in node_modules as reference.
		...
		* the stock renderer read as ages from 1970. */
		...
		const ARXA_DECO_FOR = (relPath, kind) => {
			const m = kind === "dir" ? arxaDeco.dirs : arxaDeco.files;
		...
		const ARXA_DECO_BADGE = (relPath, kind) => {
			const letter = ARXA_DECO_FOR(relPath, kind);
			if (!letter) return null;
			return (0, react_jsx_runtime.jsx)("span", {
				className: "aXa_deco aXa_deco_" + letter,
				title: orgT(ARXA_DECO_TITLE[letter] || "rows.deco.modified"),
				"aria-label": orgT(ARXA_DECO_TITLE[letter] || "rows.deco.modified"),
				children: letter,
			}, "deco");
		};
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=arxa-sidebar-empty-state]") === null) {
			...
			tag.textContent = "[data-arxa-hero-guide]{font-size:12.5px;opacity:.72;line-height:1.55;max-width:470px}"
				...
				+ ".aXa_fs_section{margin-top:6px;border-top:.5px solid var(--dsw-alias-border-l2);padding-top:4px}.aXa_fs_disclosure{display:flex;align-items:center;width:100%;padding:6px 10px;border:0;background:transparent;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xs-13);text-align:left;cursor:pointer}.aXa_fs_disclosure:hover{color:var(--dsw-alias-label-primary)}.aXa_fs_subtle{padding:4px 12px;color:var(--dsw-alias-label-caption);font:var(--dsw-font-xs-13)}"
		...
		* reserved for server-truth changes. */
		...
		function WelcomeGate({ t }) {
			...
			const freestyle = useFreestyle();
			const [freestyleError, setFreestyleError] = (0, react.useState)(null);
			// 2026-09-08: this gate covers the WHOLE app when the org count hits
			// zero — including the Trash row. Trash your last org and the only
						...
						(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: 10, justifyContent: "center" }, children: [
							...
							(0, react_jsx_runtime.jsx)("button", {
								onClick: (e) => {
									...
									freestyleStore.setTab("freestyle").catch((error) => setFreestyleError(String((error && error.message) || error)));
								},
								onKeyDown: (e) => e.stopPropagation(),
								style: { border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 8, padding: "8px 18px", fontSize: 13, cursor: "pointer", background: "var(--dsw-alias-bg-layer-2)", color: "var(--dsw-alias-label-primary)" },
								children: t("welcome.freestyle")
							}),
							(0, react_jsx_runtime.jsx)("button", {
								disabled: true,
...
"menu.org.trash": "Move to Trash",
			...
			"welcome.freestyle": "Use Freestyle",
		...
		"menu.org.trash": "Przenieś do Kosza",
			...
			"welcome.freestyle": "Użyj Freestyle",
		...
		"menu.org.trash": "Déplacer vers la corbeille",
			...
			"welcome.freestyle": "Utiliser Freestyle",
