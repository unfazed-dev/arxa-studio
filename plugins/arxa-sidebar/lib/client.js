		...
		* the stock renderer read as ages from 1970. */
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=arxa-sidebar-empty-state]") === null) {
			...
			tag.textContent = "[data-arxa-hero-guide]{font-size:12.5px;opacity:.72;line-height:1.55;max-width:470px}"
				...
				// Q5 (revised 2026-09-03): the preset chip sits at the composer's
				// top-right. dsh renders the agentPreset slot wrapper display:contents,
				// so a margin on the wrapper (the old :last-child rule) was a no-op and
				// the chip stayed left — push the CHIP, the wrapper's child. The 16px
				// inset mirrors the card's own inset inside the composer stack (780px
				// card centered in 812px). Bound state only: the empty state centers
				// the guide copy in this same row.
