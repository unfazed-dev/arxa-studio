...
try {
  ...
  ok(result.result?.finish?.can === false && result.result?.finish?.reason === 'worktree-dirty', 'Finish dry-run reads the no-org Freestyle owning repo')
  ...
  result = await act('card.status', { sessionId: dshId })
  ok(result.ok === true && result.result?.finish?.can === true, 'Finish dry-run enables after the Freestyle session lands cleanly')
  result = await act('card.finish', { sessionId: dshId })
  ok(result.ok === true && result.result?.finished === true && result.result?.id === session.id, 'card.finish accepts the dsh id with no open organisation')
  ok(!fs.existsSync(session.worktree), 'card.finish removes the Freestyle worktree')
  ok(gw.runGit(['show-ref', '--verify', '--quiet', 'refs/heads/' + session.branch], { cwd: rootPath, env, allowFail: true }) === null, 'card.finish deletes the Freestyle session branch from its owning repo')
