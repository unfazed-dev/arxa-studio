...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#endregion
		//#region arxa: locales pl+fr (stock locales/zh.js — zh dropped)
				/** pl base dictionary for the common namespace, checked complete against the en key set. */
		const pl$1 = {
			"ok": "OK",
			"cancel": "Anuluj",
			"close": "Zamknij",
		...
		/** fr base dictionary for the common namespace, checked complete against the en key set. */
		const fr$1 = {
			"ok": "OK",
			"cancel": "Annuler",
			"close": "Fermer",
		...
		/** en base dictionary for the common namespace, checked complete against the en key set. */
		const en$1 = {
			"ok": "OK",
			"cancel": "Cancel",
			"close": "Close",
		...
		/** French dictionary, checked complete against the en key set. */
		const fr = { "language.title": "Langue" };
		/** English dictionary, checked complete against the en key set. */
		...
		const css = ".aXa_loc_row{border-bottom:.5px solid var(--dsw-alias-border-l2);align-items:center;gap:8px;padding:16px 0;display:flex}.aXa_loc_rowText{flex-direction:column;flex:1;gap:4px;min-width:0;padding-right:48px;display:flex}.aXa_loc_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}.aXa_loc_selector{background:var(--dsw-alias-bg-module-platform);height:36px;font:inherit;color:var(--dsw-alias-label-primary);cursor:pointer;border:none;border-radius:18px;align-items:center;gap:12px;padding:0 14px;font-size:14px;line-height:22px;display:inline-flex}.aXa_loc_selector:hover{background:var(--dsw-alias-interactive-bg-hover)}.aXa_loc_chevron{flex:none}";
		const tagId = "arxa-locale/LanguageRow.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "arxa-locale";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var LanguageRow_module_css_default = {
			"chevron": "aXa_loc_chevron",
			"row": "aXa_loc_row",
			"rowText": "aXa_loc_rowText",
			"selector": "aXa_loc_selector",
			"title": "aXa_loc_title"
		};
		//#endregion
		...
		*/
		...
		/** The three locales and dictionaries arxa ships — stock derives this list from LOCALE_IDS × metadata; the mirror spells it out (dsh 0.1.2-rc.1). */
		const BUILT_IN_LOCALES = Object.freeze([
			Object.freeze({ id: "en", label: "English" }),
			Object.freeze({ id: "pl", label: "Polski", fallback: "en" }),
			Object.freeze({ id: "fr", label: "Français", fallback: "en" })
		]);
		...
		*/
		...
		function syncDocumentLanguage(snapshot) {
			...
			document.documentElement.lang = DOCUMENT_LANGUAGE[snapshot.active] ?? snapshot.active;
		}
		/**
